---
description: sjmj-ai invoice-ocr API 규칙 — api-spec.json 동기화, 라우터 구조, 무인증 경계, 구조화 envelope, PHP 동형 골든 계약
paths:
...
이 프로젝트의 백엔드 API는 PHP SJMJ-Web을 **동형 포팅**한 FastAPI 서버다(`apps/invoice-ocr/backend`).
요청 흐름은 **router → service → repository** 3계층이며, 검증·응답·에러 계약은 PHP 원본과의 동치성을 골든 테스트로 보존한다.
...
- Validator 규칙·골든 메시지 변경 → 해당 요청 스키마 description + 관련 테스트
...
- 요청 body 는 `dict = Body(...)` **free-form** 으로 받고 `core/validators.Validator` (fluent)로 검증한다. **Pydantic 모델을 쓰지 않는 것은 의도된 선택** — PHP `core/Validator.php` 의 골든 에러 메시지·`details` 형태를 문자 단위로 보존하기 위함이다.
...
`core/errors.py` 단일 형태(PHP `Response::error` 동형):
...
- **DELETE 는 204 가 아니라 200 + `{success, data:null, message}`** 를 반환한다(PHP 동형). 새 DELETE 도 이 형태를 따른다.
...
3. **가짜 pagination** — `GET /api/companies/{id}/invoices`(page1/limit9999/totalPages1)와 `GET /api/salespeople`(page1/limit=total/totalPages1)은 전건을 반환하며 pagination 메타는 형식만 채운다(PHP 컨트롤러 동형).
...
- Breaking change 시: PHP 동치 계약(메시지·응답 포맷·부수효과)을 깨는지 먼저 확인하고 골든 테스트로 검증한다.
...
- **거래명세서 합계는 서버가 계산하지 않는다.** `total_supply/total_vat/grand_total` 과 품목 `supply/vat/total` 은 **클라이언트(프론트 invoice-form)가 계산해 전송**하며, 서버(service/repository)는 전송값을 그대로 저장하고 누락 시 0으로 저장한다(PHP 동형). spec 기반 클라이언트가 이 필드를 생략하면 합계가 0으로 저장되므로 주의.
- `POST/PUT /api/invoices` 는 거래처·품목의 `usage_count` 를 증가시킨다(PHP modern 부수효과 동치). 명세 변경 시 이 부수효과 유지 여부를 명시한다.
...
- `quantity` 는 PHP `is_int` 동형 검증: bool 불가, 정수 문자열("100") 허용, 0~999,999,999.
...
- 골든 계약(검증 메시지 문자열·응답 포맷·부수효과)을 깨는 변경은 contract 테스트로 보호한다.
