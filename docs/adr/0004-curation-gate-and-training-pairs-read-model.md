...
- **잠금 순서는 `ocr_jobs` → `training_pairs`로 고정한다.** `mark_reviewed`와 `patch_pair` 두 **사람 경로**, 그리고 기계 apply 스크립트(`ml/tools/blank_crop_report.py`의 `build_apply_script`) — **세 경로 모두 부모→자식으로 통일됐다**([#76](https://github.com/GangsubLim/sjmj-ai/issues/76)). 기계 경로는 #52 시점까지 한 트랜잭션에서 `training_pairs` → `ocr_jobs`(자식→부모)라 사람 경로와의 순환 대기가 가능했고(#52 이전부터 있던 위험, 오퍼레이터 단발 실행 + InnoDB 교착 감지로 수용), #76에서 첫 문으로 대상 잡 행을 정렬된 id 순서로 `FOR UPDATE` 잠근 뒤 쌍 UPDATE로 내려가도록 정렬해 해소했다.
