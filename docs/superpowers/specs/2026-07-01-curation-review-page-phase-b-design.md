...
// 잡 상세(GET /jobs/{id})의 pair — top5 포함, job_id 없음.
interface CurationJobPair extends CurationPairBase {
  top5: CurationTop5Item[];
}

// PATCH /pairs/{id} 응답 — job_id 포함, top5 없음(계약 비대칭).
interface CurationPairPatchResult extends CurationPairBase {
  job_id: number;
}
...
→ `patchPair` 발행 → 성공이면 응답(`CurationPairPatchResult`)을 기존 로컬
`CurationJobPair`에 **spread merge**(top5는 PATCH 응답에 없으므로 기존 값 보존),
실패면 이전 값 롤백 + sonner 에러 토스트. invoice는 절대 건드리지 않는다(분리 불변식).

## 4. UI 페이지

### 4.1 `CurationQueuePage` (`/curation`)

confirmed 잡 목록. 페이지네이션(`ui/pagination` 재사용).
...
- **미검수 우선순위는 서버 정렬로 처리** — repo가 `curation_reviewed ASC`로 정렬해
  미검수 잡이 목록 상단에 온다(`curation_repository.list_jobs`). 별도 "미검수만 보기"
  필터는 두지 않는다: `/api/curation/jobs`가 `page`/`limit`만 받으므로 클라이언트
  필터는 현재 페이지만 걸러 `total`/`totalPages`를 거짓으로 만든다. 상태별 서버 필터가
  필요해지면 Phase A 계약(쿼리 param)을 확장하는 후속으로 분리한다(§6).
...
- **vitest 단위**: `use-curation-jobs`/`use-curation-job`(옵티미스틱 PATCH 성공·롤백,
  PATCH 응답 merge 시 top5 보존), `curationAPI` URL 빌더, 변경 강조 판정 로직.
- **playwright e2e (mock)**: 현재 `playwright.config.ts`는 `VITE_USE_MOCK=true`로 mock
  Vite 서버만 띄우고 `ocrAPI`는 mock proxy 없이 real 고정이다(api.ts). 따라서 Phase B
  e2e는 **mock 큐레이션 페이지 플로우**로 한정한다: mock curationAPI가 시드한 잡으로
  `/curation` 큐 진입 → 드릴다운(라벨편집·제외, mock PATCH) → 검수완료(mock review)
  → 큐에서 ✓ 확인. 이미지는 mock 불가라 placeholder degrade를 검증한다.
- **라이브 OCR 관통(업로드→confirm→큐레이션)**: 현재 e2e 설정으로 mock 모드에선 불가.
  실 backend+DB seed를 전제로 하는 별도 runbook(또는 `test:e2e:live` 설정)으로 분리한다
  — Phase B 자동 e2e 범위 밖.

## 6. 비범위 (Phase B)

- "유사 기존 학습 라벨 힌트"(신규 distinct-canonical EP 필요) → Phase C.
- 상태별 서버 필터("미검수만" 등 `/api/curation/jobs` 쿼리 param) → Phase A 계약 확장
  후속(현재는 미검수-우선 정렬로 갈음).
- 라이브 OCR 관통 e2e(실 backend+DB seed) → 별도 runbook.
