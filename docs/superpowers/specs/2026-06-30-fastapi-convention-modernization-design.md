...
| 구분            | 항목                                                                                                                                                                                                                                                                                                                                       |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **불변 (계약)** | 성공 envelope shape `{success, data, pagination?}` · 에러 envelope shape `{success, error: {code, message, details?}}` · 에러 코드 체계(`VALIDATION_ERROR`/`NOT_FOUND`/`DUPLICATE_NAME`/`CONFLICT`/`SERVER_ERROR`) · 검증 실패 HTTP status = **400** · `details` 형태 = `{필드: 메시지}` 문자열 맵(`api-spec.json` `ErrorEnvelope`가 SSoT) |
| **자유 (구현)** | 검증 메시지 문자열 내용 · 내부 검증 방식(Validator → Pydantic) · request 모델 구조                                                                                                                                                                                                                                                         |

`details` 형태를 **불변으로 둔 이유**: 프론트가 현재 소비하지 않아 바꿀 동인이 없고, `api-spec.json`이 이미 `{필드: 메시지}` 문자열 맵으로 고정하고 있다(드리프트 방지). 형태를 자유화하면 SSoT와 어긋날 뿐 이득이 없다. 향후 프론트가 필드별 검증 표시를 도입하면 그 형태 그대로 계약이 굳는다.

**Pydantic 검증 실패 핸들러(선결 작업).** 현재 `app/core/errors.py:register_error_handlers`는 `AppError`와 미처리 `Exception`만 등록한다 — `RequestValidationError` 핸들러가 없어, request 모델을 Pydantic으로 바꾸면 검증 실패가 FastAPI 기본 422 경로로 빠진다(계약 위반). 따라서 **첫 Pydantic 슬라이스는 그 전에(또는 그 일부로) `RequestValidationError` 핸들러를 추가**해 ① Pydantic 에러 리스트를 `{필드: 메시지}` `details`로 매핑하고 ② 400 `{success, error: {code: "VALIDATION_ERROR", message, details}}` envelope로 변환한다. 이 핸들러의 동작(400·코드·details 형태)을 고정하는 contract 테스트를 함께 추가한다. 이는 슬라이스 1회성 비용이 아니라 모든 후속 Pydantic 슬라이스가 공유하는 cross-cutting 인프라다.
...
2. **frontend legacy 제거** — `api.ts` legacy 분기 9곳(~121줄) 제거: `ep()` → 고정 `/${resource}`, settings의 localStorage fallback, company/item update의 delete+create fallback, invoice duplicate fallback, export modern-only 체크 제거. `VITE_API_MODE` env·타입 제거. `types/api.ts`의 legacy 호환 필드(`total?`/`page?`/`limit?`) 제거. `mocks`의 ListResponse를 `pagination` 중첩 구조로 단일화(6곳).
   - **기본값 변경 주의**: `api.ts`의 현재 `API_MODE`는 `?? "legacy"` 기본값이다(.env가 `modern`을 지정해야 modern). `VITE_API_MODE` 제거는 경로를 **modern으로 하드코딩**하므로, .env를 누락한 빌드에서는 legacy→modern 기본값 변경이다. 단 legacy 경로 `/{resource}.php`는 FastAPI 백엔드에 존재하지 않아 이미 404로 비작동 → 실배포는 무영향이다. Phase 0에서 `.env.example`·dev(vite)·prod(launchd)·CI 빌드가 legacy 기본값에 의존하지 않음을 확인하고 제거한다.
...
- **검증 실패를 Pydantic 기본 422로 전환.** 당장은 외부 계약 보존을 위해 400을 유지하지만, 추후 프론트가 **검증 status(400)에 분기 의존하지 않음**을 재확인한 뒤 Pydantic 표준 422 + 표준 에러 바디로 갈아탈 예정이다. 그 시점에 envelope 래핑 자체를 재검토한다. 주의: 프론트는 검증 status에는 의존하지 않지만 **다른 status에는 의존한다** — `invoice-form.tsx`가 HTTP 409(CONFLICT, 이미 확정된 명세서)로 분기한다. 따라서 이 전환의 전제는 "status 의존 전무"가 아니라 좁게 "**검증 status(400) 의존 전무**"이며, 409 등 비검증 status 분기는 그대로 유지된다.
