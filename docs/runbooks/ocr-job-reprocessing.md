...
- 진행은 워커 로그로 본다. **창구가 하나가 아니다** — 잡별 진행 신호(`[warp-gate]`)는
  `flush=True`만 걸려 있어 stdout(`ml-worker.out.log`)으로 가고, 부팅 지문과 크롭 교체
  실패는 `file=sys.stderr`로 명시돼 stderr(`ml-worker.err.log`)로 간다. 둘 다 본다:

  ```bash
  tail -f ~/.sjmj-ai/logs/ml-worker.out.log ~/.sjmj-ai/logs/ml-worker.err.log
  ```

  (`SJMJ_LOG_DIR`로 재정의하면 경로가 달라진다.)
...
- 나오면 **그 잡을 다시 재처리해서**(3단계) 교체를 끝낸다. 재처리는 멱등이라 같은 사진·같은
  엔진이면 좌표가 제자리에 남는다. 워커가 처리 중일 때도 `.tmp`가 보이므로, 먼저 워커 로그로
  진행 중인지 확인한다.
...
- **추론 도중(커밋 전)에 프로세스가 죽으면 잡은 `running`에서 멈춘다** — `pending → running`
  ...
  거부돼 그대로는 3단계로 돌아갈 수 없다. **워커가 실제로 그 잡을 처리 중이 아님을 먼저
  확인한다**(워커 로그에 그 잡 id가 최근에 없는지, 워커 프로세스가 살아 있는지) — 처리
  중인데 상태를 바꾸면 이중 처리가 된다. 확인 후에만 수동으로 되돌린다:
...
| 잡이 `running`에서 멈춘 채 진행이 없음           | 워커가 추론 도중 죽었다(워치독 없음). 처리 중이 아님을 확인한 뒤 상태를 `done`으로 되돌리고 3단계부터 재시도(5단계 참조) |
