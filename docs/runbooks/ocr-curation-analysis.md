...
`fetch`가 서버에서 당겨 캐시에 굳히는 것은 다섯이다.
...
**교정 이력(`ocr_corrections`)에서 읽는 것은 아직 행 수지뿐이다.** 사람이 행을 몇 개 보태고
몇 개 버렸는지(행 수지)는 이제 `corrections.json`으로 읽어 리포트 `## 행 수지` 절에 낸다.
반면 라벨을 top-1 그대로 뒀는지 후보에서 골랐는지 직접 쳤는지(조작 출처,
`correction_json.lines[].label_source`)는 DB에 남지만 아직 읽지 않는다 — 다음 슬라이스가
이 캐시를 그대로 재사용한다.
