...
`fetch`가 서버에서 당겨 캐시에 굳히는 것은 넷이다.
...
**교정 이력(`ocr_corrections`)은 소스가 아니다.** 사람이 행을 몇 개 보태고 몇 개 버렸는지
(행 수지), 라벨을 top-1 그대로 뒀는지 후보에서 골랐는지 직접 쳤는지(조작 출처)는 DB에 남지만
이 리포트는 읽지 않는다.
...
| 빠지는 것               | 근거                                             | 성격                                                                                           |
| ----------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| 사람이 손으로 추가한 행 | 크롭 좌표가 없는 줄은 학습 후보 쌍이 되지 않는다 | **눈이 머는 것** — 모델이 놓친 행이 분자에도 분모에도 없어, 행검출이 나빠질수록 남은 행만 센다 |
