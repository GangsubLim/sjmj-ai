...
def test_store_upsert_creates_201_structured(client):
    ...
    r = client.post("/api/sales-records", json={
        "salesperson_id": sp_id, "work_date": "2026-05-15", "quantity": 1000000,
    })
...
def test_store_ignores_client_snapshot_name(client):
    ...
    r = client.post("/api/sales-records", json={
        "salesperson_id": sp_id, "work_date": "2026-05-15", "quantity": 100,
        "snapshot_name": "조작된이름",
    })
...
def test_store_upsert_updates_existing(client):
    sp_id = _insert_salesperson()
    client.post("/api/sales-records", json={"salesperson_id": sp_id, "work_date": "2026-05-15", "quantity": 100})
    r = client.post("/api/sales-records", json={"salesperson_id": sp_id, "work_date": "2026-05-15", "quantity": 200})
    assert r.status_code == 201
...
def test_store_unknown_salesperson_404(client):
    r = client.post("/api/sales-records", json={
        "salesperson_id": 99999, "work_date": "2026-05-15", "quantity": 100,
    })
...
def test_store_rejects_over_max_quantity(client):
    sp_id = _insert_salesperson()
    r = client.post("/api/sales-records", json={
        "salesperson_id": sp_id, "work_date": "2026-05-15", "quantity": 1000000000,
    })
    assert r.status_code == 400
    assert r.json()["error"]["details"]["quantity"] == "out of range"
...
def test_store_rejects_non_integer_quantity(client):
    sp_id = _insert_salesperson()
    r = client.post("/api/sales-records", json={
        "salesperson_id": sp_id, "work_date": "2026-05-15", "quantity": 1.5,
    })
    assert r.status_code == 400
...
def test_index_returns_single_aggregate_no_pagination(client):
    sp_id = _insert_salesperson()
    client.post("/api/sales-records", json={"salesperson_id": sp_id, "work_date": "2026-05-15", "quantity": 1000})
...
def test_index_salespeople_includes_active_and_record_holders(client):
    ...
    client.post("/api/sales-records", json={"salesperson_id": inactive_id, "work_date": "2026-05-10", "quantity": 500})
...
def test_destroy_removes_then_404(client):
    sp_id = _insert_salesperson()
    client.post("/api/sales-records", json={"salesperson_id": sp_id, "work_date": "2026-05-15", "quantity": 100})
    rec = client.get("/api/sales-records", params={"year": 2026, "month": 5}).json()["data"]["records"][0]
