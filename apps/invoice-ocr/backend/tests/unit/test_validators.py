...
def test_max_length_multibyte():
    ...
    # 정확히 100자는 통과
    assert (
        Validator().max_length({"recipient": "가" * 100}, "recipient", 100).fails()
        is False
    )
...
def test_date_format():
    assert (
        Validator().date_format({"issue_date": "2026-05-15"}, "issue_date").fails()
        is False
    )
    ...
    # 존재하지 않는 날짜도 거부
    assert (
        Validator().date_format({"issue_date": "2026-13-40"}, "issue_date").fails()
        is True
    )
...
def test_business_number():
    assert (
        Validator().business_number({"b": "123-45-67890"}, "b").fails() is False
    )  # 10자리
...
def test_numeric():
    ...
    assert (
        Validator().numeric({"n": "abc"}, "n").errors["n"] == "n은(는) 숫자여야 합니다."
    )
