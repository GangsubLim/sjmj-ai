...
def store(data: dict = Body(...)):
    Validator().required(data, ["salesperson_id", "work_date", "quantity"]) \
        .date_format(data, "work_date") \
        .validate_or_fail()
    ...
    if quantity_int < 0 or quantity_int > _MAX_QUANTITY:
        bad_request("quantity는 0 이상 999,999,999 이하여야 합니다.", {"quantity": "out of range"})
