...
describe("ItemOcrCandidates", () => {
  it("모든 행에 지연 로딩 크롭 썸네일을 보여준다", () => {
    render(
      <ItemOcrCandidates meta={meta()} selectedLabel="" onPick={vi.fn()} />,
    );
  ...
  // 접힘 상태에서 목록 컨테이너를 아예 마운트하지 않으면 aria-controls가 존재하지 않는
  // id를 가리켜 IDREF가 해소되지 않는다(axe aria-valid-attr-value 계열 위반).
  it("접힌 상태에서도 aria-controls가 가리키는 요소가 존재한다", () => {
    const { container } = render(
      <ItemOcrCandidates meta={meta()} selectedLabel="" onPick={vi.fn()} />,
    );
    const listId = screen
      .getByRole("button", { name: /후보/ })
  ...
  it("현재 품목명과 같은 후보 칩만 선택 상태(aria-pressed)로 노출한다", () => {
    render(
      <ItemOcrCandidates
        meta={meta({ uncertain: true })}
        selectedLabel="배터리"
    ...
    expect(screen.getByRole("button", { name: /배터리/ })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    expect(screen.getByRole("button", { name: /타이어/ })).toHaveAttribute(
      "aria-pressed",
      "false",
    );
  });
