...
class WorkerQueue:
    ...
    def fetch_pairs(self, job_id: int) -> list[OldPair]:
        ...
        **조인으로 인한** 오결합과 봉인이 함께 사라진다. 정렬로 인한 오결합은 남는다 —
        row_index는 여전히 plan_relink의 정렬 축인데 commit_job ①이 미결 전환에서 그것을
        갱신하지 않아 낡은 채로 남고, 이제 미결 쌍도 앵커를 들고 ②에 참여하기 때문이다.
        미결 쌍이 ①에서 이미 같은 낡은 인덱스로 정렬돼 온 성질이 ②로 넓어진 것이라 새 결함은
        아니며, 알고도 남겨 둔 잔여 위험이다(승계 알고리즘 변경은 이 이슈의 범위 밖).
