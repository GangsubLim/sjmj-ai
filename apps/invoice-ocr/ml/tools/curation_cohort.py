...
분리). 리포트 본체는 fetch 글루에 렌더까지 담아 파일 상한(800줄)에 닿았는데, 이 계층은 IO 0·
부수효과 0이라 합성 데이터 단위테스트로 전량 닫히므로 경계가 자연스럽다(이후 렌더 계층도
tools/curation_render.py로 같은 이유로 갈렸다 — Issue #38).
