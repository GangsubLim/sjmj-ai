...
수기(손글씨) 거래명세서 OCR. 오프라인 배치 CLI + 배포된 추론 worker. 외부 API 0, 전 과정 로컬.
...
handwriting/    손글씨 인식 추론 (git-tracked, production). worker가 import해 실행
...
tools/          spike_ppstructure.py — 환경/검출 스파이크 (paddle 필요)
                ...
                curation_label_source.py — 위 리포트의 조작 출처 계층 (label_source 조회 SQL·파싱·분포·교차, 순수함수만)
                curation_render_label_source.py — 위 리포트의 조작 출처 렌더 (## 조작 출처 + ### 출처 × 품목 버킷, 순수함수만)
                curation_render_fmt.py — curation_render·curation_render_label_source가 공유하는 서식 원자 (pct/share/known_text, 순환 방지)
...
git-tracked는 `ocr_poc/ handwriting/ worker/ tests/ tools/ pyproject.toml README.md .env.example`. 나머지(report/ results/ review/ data/ poc/ .venv/)는 전부 로컬·gitignore.
