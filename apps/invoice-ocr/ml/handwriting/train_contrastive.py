...
def prepare(corr):
    ...
    key = hashlib.md5(("v2" + json.dumps(corr, sort_keys=True, ensure_ascii=False)).encode()).hexdigest()[:10]
    if CACHE.exists():
    ...
    for r in rows:
        ...
        inv.append(r["cn"])
        keys.append(k)
    np.savez(CACHE, key=key, sq=np.array(sq), lab=np.array(lab, object),
             inv=np.array(inv, object), keys=np.array(keys, object))
...
def split_invoices(inv, labels, recurring, val_frac=0.25, rng=None):
    ...
    rng.shuffle(invs)
    # 전표별 보유 재현라벨
    inv_labs = {iv: {labels[i] for i in range(len(inv)) if inv[i] == iv and labels[i] in recurring} for iv in invs}
...
def build_model(device):
    ...
    vit = VisionEncoderDecoderModel.from_pretrained("ddobokki/ko-trocr", torch_dtype=torch.float32).encoder
...
# ---------------- 학습 (단일 split) ----------------
def train_split(base, ids, lab_arr, inv_arr, recurring, tr, va, lab2id, args, device,
    ...
    if verbose:
        bn = ob["n"]
        print(f"  베이스라인 backbone: top1 {ob['t1']/max(bn,1):.0%} top3 {ob['t3']/max(bn,1):.0%} "
              f"top5 {ob['t5']/max(bn,1):.0%} (n{bn})")

    crit = SupConLoss()
    enc_p = [p for p in model.enc.parameters() if p.requires_grad]
    opt = torch.optim.AdamW([{"params": enc_p, "lr": 2e-5},
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for s in range(steps):
            ...
            if len(idx) < 2:
                continue
            b = base[idx]
            x = torch.cat([torch.stack([TRAIN_TF(z) for z in b]),
                           torch.stack([TRAIN_TF(z) for z in b])]).to(device)
        ...
        pj = retrieval(zq, lab_arr[va], inv_arr[va], zb, lab_arr[tr], inv_arr[tr], recurring)   # projection
        bb = retrieval(hq, lab_arr[va], inv_arr[va], hb, lab_arr[tr], inv_arr[tr], recurring)   # backbone
        mk = ""
        ...
        if verbose:
            nv = pj["n"]
            print(f"  ep{ep:>2} loss {tot/steps:.3f} · proj {pj['t1']/nv:.0%}/{pj['t3']/nv:.0%}/{pj['t5']/nv:.0%} "
...
def train_production(base, ids, lab_arr, inv_arr, keys, lab2id, args, device):
    ...
    np.savez(BANK, emb=Z, lab=np.array(lab_arr, object), inv=np.array(inv_arr, object),
...
def main():
    ...
    print(f"교정: drop {len(corr.get('drop',[]))} · ditto {len(corr.get('ditto',[]))} · "
    ...
    print(f"clean {len(sq)} crop · 라벨 {len(set(lab))} · 재현(≥2전표) {len(recurring)} · 전표 {len(set(inv))}")

    base = T.functional.to_dtype(torch.stack([T.functional.to_image(s) for s in sq]),
                                 torch.float32, scale=True)   # [N,3,224,224] in [0,1]
    ...
    if args.folds <= 1:
        ...
        splits = [(set(g.tolist()), f"fold{k+1}") for k, g in enumerate(np.array_split(invs, args.folds))]
    ...
    for val_set, tag in splits:
        ...
        results.append(train_split(base, ids, lab_arr, inv_arr, recurring, tr, va,
                                   lab2id, args, device, ckpt=ck, verbose=True))
    ...
    b1, _ = rate("base", "t1"); b3, _ = rate("base", "t3"); b5, _ = rate("base", "t5")
    p1, _ = rate("p", "t1"); p3, _ = rate("p", "t3"); p5, _ = rate("p", "t5")
    bb1, _ = rate("bb", "t1")
    ...
    print(f"파인튜닝 projection     : top-1 {p1:.1%} · top-3 {p3:.1%} · top-5 {p5:.1%}  (backbone top-1 {bb1:.1%})")
    print(f"상승 Δtop-1 {p1-b1:+.1%} · Δtop-3 {p3-b3:+.1%} · Δtop-5 {p5-b5:+.1%}")
    print("운영 시나리오(자동채움 + 사용자 검수 드롭다운):")
    print(f"  · 신뢰도 게이팅 자동채움 — 정밀도 ≥95%: 커버리지 {cov95:.0%} (실측 {prec95:.0%}) · ≥90%: {cov90:.0%}")
    print(f"  · 나머지 행은 top-{3 if p3>=p5 else 5} 드롭다운으로 사용자 선택(상위 후보 적중 top-3 {p3:.0%}/top-5 {p5:.0%})")
    REPORT.write_text(json.dumps({
        "mode": mode, "scored": BN, "scenario": "input-assist + HITL (추론 시 DB 비어있음 → 개방 retrieval)",
        "baseline_frozen_backbone": {"top1": round(b1, 4), "top3": round(b3, 4), "top5": round(b5, 4)},
        "finetuned_projection": {"top1": round(p1, 4), "top3": round(p3, 4), "top5": round(p5, 4)},
        "autofill_conf_gate": {"prec95_coverage": round(cov95, 4), "prec90_coverage": round(cov90, 4)},
