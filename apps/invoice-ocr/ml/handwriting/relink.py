...
class OldPair:
    ...
    `draft_supply`는 믿을 수 있을 때만 실린다. 초안 형식이 깨진 경우는 물론, **지난 재처리에서
    미결로 남아 row_index가 낡은 쌍**도 None이어야 한다 — 낡은 인덱스가 새 초안의 행 범위 안이면
    그 자리엔 다른 행의 값이 들어 있어, 그 가짜 앵커가 ②단계 빈칸에서 우연히 맞는 순간 확정
    라벨이 전혀 다른 줄의 그림에 붙는다. 이 판별은 좌표를 아는 공급자(worker/db.fetch_pairs)의
    책임이고, 여기서는 None 앵커가 서로 절대 같지 않다는 것만으로 회수에서 빠진다.
    """
