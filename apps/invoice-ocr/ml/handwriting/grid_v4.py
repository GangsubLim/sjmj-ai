...
def blue_mask_enh(bgr):
    """대비향상 파랑마스크 — 과노출로 (b−r)이 옅어진 흐린 격자를 복구한다.

    표준 blue_mask의 고정 임계(b−r>10)가 바랜 청색 격자를 놓치는 것을, (b−r) 양수부를
    per-image 정규화 + CLAHE + 적응 임계로 '상대적' 파랑까지 살린다. 노이즈는 hline_ys의
    가로 morphology가 걸러 격자선만 남는다(흐린격자 1→14선 실측). per-sheet 흐림 회수에서만
    쓴다(아래 _FAINT).
    """
...
class FaintOn:
    """with FaintOn(cn in FAINT_SET): ... — 블록 안에서만 hline_ys가 대비향상 마스크 사용."""

    def __init__(self, on):
        """on이 참인 동안만 흐림 회수를 켜는 컨텍스트 매니저를 만든다."""
        self.on = bool(on)

    def __enter__(self):
        """현재 _FAINT를 저장하고 on 값으로 바꾼다."""
        global _FAINT
        self.prev = _FAINT
        _FAINT = self.on

    def __exit__(self, *exc):
        """저장해 둔 이전 _FAINT 값으로 복원한다."""
        global _FAINT
        _FAINT = self.prev
...
def form_quad(mask):
    """폼 사각형을 검출한다 — 극점(원근보정)이 기본, quad가 깨지면 minrect로 폴백한다.

    한 코너가 안쪽으로 끌리면(마스크 누락·배경오염 — inv_001) 깨짐으로 판정. 극점은 정밀하나
    마스크 누락에 취약, minrect는 강건하나 원근 손실 → 깨짐 판정으로 양쪽 장점만 취한다.
    """
...
def warp(bgr, quad):
    """quad를 템플릿 크기(WARP_W×WARP_H)로 원근 워프한다."""
...
def hline_ys(warped):
    """전체 표를 가로지르는 수평 파랑 격자선의 y들을 반환한다.
...
def main(img_id):
    """전표 한 장을 워프·행분할해 공급가 셀 crop과 오버레이를 저장한다."""
