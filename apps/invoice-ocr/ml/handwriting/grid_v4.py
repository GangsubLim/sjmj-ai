...
def _order(pts):
    ...
    return np.array([pts[s.argmin()], pts[d.argmax()], pts[s.argmax()], pts[d.argmin()]], np.float32)
...
def _hlines_from_mask(m):
    horiz = cv2.morphologyEx(m, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_RECT, (WARP_W // 3, 1)))
...
def main(img_id):
    ...
    for a, b in zip(ys, ys[1:]):
        ...
        k = len(cells)
        cv2.imwrite(str(CROPS / f"{img_id}_amt_{k:02d}.png"), warped[a - 3:b + 3, x1 - 4:x2 + 4])
        cells.append((a, b))
