...
def _quad_derot(c):
    ...
    corners = np.array(
        [rot[s.argmin()], rot[d.argmax()], rot[s.argmax()], rot[d.argmin()]], np.float32
    )
...
def _line_spread(warped):
    ...
    lines = cv2.HoughLinesP(
        m, 1, np.pi / 360, threshold=120, minLineLength=WARP_W // 4, maxLineGap=30
    )
    if lines is None:
        return 6.0
...
def deskew_angle(warped):
    ...
    m = blue_mask(warped)
    lines = cv2.HoughLinesP(
        m, 1, np.pi / 360, threshold=150, minLineLength=WARP_W // 3, maxLineGap=30
    )
    if lines is None:
        return 0.0
    angs = []
