...
DS = Path(sys.argv[2]) if len(sys.argv) > 2 else \
    Path("/Users/gangsub/projects/sjmj-ai/apps/invoice-ocr/ml/report/dataset")
...
def embed(items, proc, model, device, is_trocr):
    ...
    for _, _, f in items:
        ...
        x = proc(images=square(img), return_tensors="pt").to(device) if not is_trocr \
...
def main():
    ...
    eval_labels = [labels[i] for i in range(len(items))
                   if labels[i] in recurring and any(labels[j] == labels[i] and invs[j] != invs[i] for j in range(len(items)))]
    ...
    print(f"top-1: {top1}/{evaln} = {top1/evaln:.1%}  (다수라벨 베이스라인 {base_major:.1%})" if evaln else "top-1: n/a")
